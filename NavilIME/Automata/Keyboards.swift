...
class Keyboard {
    ...
    var input_delta_threshold: TimeInterval
    var prev_chosung_input_time: TimeInterval
    var prev_jungsung_input_time: TimeInterval
    var prev_jongsung_input_time: TimeInterval
    var chosung_input_delta: TimeInterval
    var jungsung_input_delta: TimeInterval
    var jongsung_input_delta: TimeInterval
    ...
    init() {
        ...
        self.input_delta_threshold = 150
        self.prev_chosung_input_time = Date().timeIntervalSince1970
        self.prev_jungsung_input_time = Date().timeIntervalSince1970
        self.prev_jongsung_input_time = Date().timeIntervalSince1970
        self.chosung_input_delta = 0
        self.jungsung_input_delta = 0
        self.jongsung_input_delta = 0
    }

    func update_input_delta() {
        let curr_time = Date().timeIntervalSince1970
        self.input_delta = (curr_time - prev_input_time) * 1000
        self.prev_input_time = curr_time
        // PrintLog.shared.Log(log: "0 input_delta \(self.input_delta) ms")
    }

    func clear_input_delta() {
        self.input_delta = self.input_delta_threshold + 1
    }

    func update_chosung_input_delta() {
        let curr_time = Date().timeIntervalSince1970
        self.chosung_input_delta = (curr_time - prev_chosung_input_time) * 1000
        self.prev_chosung_input_time = curr_time
        // PrintLog.shared.Log(log: "1 chosung_input_delta \(self.chosung_input_delta) ms")
    }

    func update_jungsung_input_delta() {
        let curr_time = Date().timeIntervalSince1970
        self.jungsung_input_delta = (curr_time - prev_jungsung_input_time) * 1000
        self.prev_jungsung_input_time = curr_time
        // PrintLog.shared.Log(log: "2 jungsung_input_delta \(self.jungsung_input_delta) ms")
    }

    func update_jongsung_input_delta() {
        let curr_time = Date().timeIntervalSince1970
        self.jongsung_input_delta = (curr_time - prev_jongsung_input_time) * 1000
        self.prev_jongsung_input_time = curr_time
        // PrintLog.shared.Log(log: "3 jongsung_input_delta \(self.jongsung_input_delta) ms")
    }

    func chosung_proc(comp: inout Composition, nobreak: Bool, current: [String], i: Int) -> Bool {
        self.chosung_layout[current[i]] != nil ? true : false
    }

    func jungsung_proc(comp: inout Composition, nobreak: Bool, current: [String], i: Int) -> Bool {
        self.jungsung_layout[current[i]] != nil ? true : false
    }

    func jongsung_proc(comp: inout Composition, nobreak: Bool, current: [String], i: Int) -> Bool {
        self.jongsung_layout[current[i]] != nil ? true : false
    }

    func fallback_proc(comp: inout Composition, current: [String], i: Int) {
        comp.done = true
    }
